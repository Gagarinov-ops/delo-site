...
window.registerRoom = function(pointIds, wallIds, height = 2500) {  
  ...
  const room = {  
    ...
    pointIds: [...pointIds], wallIds: [...wallIds], anchorPointId, height,  
    perimeter: 0, floorArea: 0, wallArea: 0  
  };  
...
window.registerOpening = function(wallId, roomId, type, width, height, position) {  
  ...
  const opening = {  
    id: openingId, type, width, height, area: areaM2, wallId, position,  
    offsetFromLeft: position - width/2, offsetFromRight: wall.length - (position + width/2)  
  };  
